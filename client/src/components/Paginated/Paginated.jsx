...
class Paginated extends Component {
  Actualizar() {
    ...
    let IndiceFinal =
      Math.floor(this.props.paginated.productsView.length / SizeWhachCards) +
      (this.props.paginated.productsView.length % SizeWhachCards === 0 ? 0 : 1);
    let PaginaStart =
      this.props.paginated.page - Math.floor(SizeEnumeracionPaginado / 4) < 1
        ? 1
        : this.props.paginated.page - Math.floor(SizeEnumeracionPaginado / 2);
    PaginaStart =
      IndiceFinal - PaginaStart > SizeEnumeracionPaginado - 1
        ? PaginaStart
        : IndiceFinal - (SizeEnumeracionPaginado - 1);
      ...
      <nav aria-label="Countries Pagination" className={style.paginationGlobal}>
        <ul className={style.pagination}>
          <li
            key={0}
            className="page-item"
            id={this.props.paginated.page === 1 && style.Inhabilitado}
            onClick={() =>
          ...
          {IndicesArray.map((page, index) => {
            return (
              <li
                key={index + 1}
                className="page-item"
                id={this.props.paginated.page === page && style.pagSeleccionada}
          ...
          <li
            key={IndiceFinal + 1}
            className="page-item"
            id={this.props.paginated.page === IndiceFinal && style.Inhabilitado}
            onClick={() =>
              this.changePage(this.props.paginated.page + 1, IndiceFinal)
            }
          >
