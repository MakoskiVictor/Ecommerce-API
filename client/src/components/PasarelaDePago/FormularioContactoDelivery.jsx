...
export class FormularioContactoDelivery extends Component {
  ...
  Comprobaciones() {
    //Buscar Id
    const found = this.props.calendar.find(
      (element) =>
        element.date == this.state.Dia && element.time == this.state.Hour
    );

    this.props.ChangeDelivery({
      idDate: found,
  ...
  render() {
    ...
    for (let index = 0; index < this.props.calendar.length; index++) {
      ...
      if (DATE < result2) Nuevoarray2.push(this.props.calendar[index]);
    }

    var Strings = [];
    for (let index = 0; index < Nuevoarray2.length; index++) {
      if (
        Strings.find((element) => element === Nuevoarray2[index].date) ===
        undefined
      )
    ...
    let NuevoArray3 = Nuevoarray2.filter(
      (element) => element.date === this.state.Dia
    );
    ...
    return (
      <div className={styles.containerFormulario}>
        ...
        <nav className={styles.flexDayContainer}>
          <ul>
            {Strings.map((elemento) => {
              ...
              return (
                <li
                  key={elemento}
                  onClick={() => this.changeDay(elemento)}
                  className={elemento == this.state.Dia && styles.seleccionado}
                >
                  {valor}
                </li>
              );
        ...
        {NuevoArray3 !== undefined && NuevoArray3.length > 0 ? (
          <div className={styles.hourContainer}>
            <nav className={styles.flexHourContainer}>
              <ul>
                {NuevoArray3.map((elemento) => {
                  return (
                    <li
                      key={elemento.time}
                      onClick={() => this.changeHour(elemento.time)}
                      className={
                        elemento.time == this.state.Hour && styles.seleccionado
                      }
                    >
                      {elemento.time}
                    </li>
...
function mapDispatchToProps(dispatch) {
  ...
  return {
    ...
    ChangeDelivery: (objectDelivery) =>
      dispatch(ChangeDelivery(objectDelivery)),
  };
}
