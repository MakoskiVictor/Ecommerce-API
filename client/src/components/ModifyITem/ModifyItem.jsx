...
const putStocks = async (data) => {
  try {
    await axios.put("http://localhost:3001/stock/AddStocks",
      data
    )
  }
  catch (error) {
...
const changeProduct = async (id, type, data) => {
  try {
    await axios.put(`http://localhost:3001/product/${id}?type=${type}`,
      data
    )
  }
  catch (error) {
  ...
  SelectSize(e) {
    ...
    if (e.target.checked == false) {
      ...
      if (this.props.products != undefined && this.props.products.length > 0) {
        let found = this.props.products[0].stocks.find(element => element.productSize == e.target.value)
        if (found !== undefined)
          objeto = { ...objeto, stocks: { ...this.state.stocks, [e.target.value]: found.stock } }
      }
    }
    ...
    if (this.props.products != undefined && this.props.products.length > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const foo = async () => {
            try {
              let objeto = { Sizes: this.state.stocks, idProduct: this.props.products[0].id }
              await putStocks(objeto).then(console.log("cambio Stock Exitoso"))
              await changeProduct(this.props.products[0].id, "name", { name: this.state.name })
              await changeProduct(this.props.products[0].id, "price", { price: this.state.price })
      ...
      <div>
        {(this.props.products != undefined && this.props.products.length > 0) ?
          <div>
            ...
            <p>
              <label className={(this.state.name !== this.props.products[0].name) && styles.ValueModify}>Name</label>
              <input id={"Name"} name={"Name"} value={this.state.name}
                onChange={(e) => this.ChangeProp(e.target.value, "name")} />
            </p>
            <p>
              <label className={(this.state.image !== this.props.products[0].image) && styles.ValueModify}>Image</label>
              <input id={"Image"} name={"Image"} value={this.state.image}
                onChange={(e) => this.ChangeProp(e.target.value, "image")} />
            </p>
            <p>
              <label className={(this.state.price !== this.props.products[0].price) && styles.ValueModify}>Price</label>
                    ...
                    return (
                      <li>
                        <p>
                          ...
                          <input type="number" min={0} id={`Select${elemento}`} name={elemento} value={this.state.stocks[elemento] == undefined ? 0 : this.state.stocks[elemento]}
            ...
            <button onClick={() => this.Guardar()}>Guardar Cambios</button>
          </div> :
          "NO FOUND PRODUCT"
        }
      </div>
